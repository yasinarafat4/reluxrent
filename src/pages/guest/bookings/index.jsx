...
const Bookings = () => {
  ...
  const { data: bookingsData = [], isLoading: isBookingsDataLoading } = useSWR(
    `/api/guest/bookings?type=${type}&status=${status}&page=${page}&search=${search}&startDate=${startDate}&endDate=${endDate}&property=${property}`,
    fetcher,
  );
  ...
  // if (isBookingsDataLoading) return null;
  return (
    <GuestLayout>
              ...
              <TableContainer sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, borderTopLeftRadius: 0, borderTopRightRadius: 0, width: '100%' }}>
                  ...
                  <>
                    {!isMd ? (
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {bookingsData.data.map((booking, i) => (
                            <TableRow key={i}>
                              {/* Property */}
                              <TableCell width={'20%'} sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                                <Stack direction={'row'} alignItems={'center'} gap={1}>
                                  <Box height={'50px'} width={'50px'}>
                                    <Avatar
                                      src={booking?.property?.propertyImages[0]?.image}
                                      name={booking?.property?.propertyDescription?.name}
                                      alt={booking?.property?.propertyDescription?.name}
                                      size="50"
                                      round="10px"
                                    />
                                  </Box>
                                  <Box>
                                    {booking?.property?.propertyDescription?.name ? (
                                      <Typography variant="body2" fontWeight={600}>
                                        {booking?.property?.propertyDescription?.name}
                                      </Typography>
                                    ) : (
                                      <Typography variant="body2" fontWeight={600}>
                                        Booking started {booking?.startDate && format(booking?.startDate, 'dd MMM, yyyy')}
                                      </Typography>
                                    )}
                                    <Stack direction={'row'} gap={0.3} alignItems={'center'}>
                                      ...
                                      <Stack direction={'row'} gap={0.5} alignItems={'center'}>
                                        {' '}
                                        <Avatar src={booking?.host?.image} name={booking?.host?.name} alt={booking?.host?.name} round={true} size="15" />{' '}
                                    ...
                                    <Typography variant="body2" fontSize={{ xs: 12 }}>
                                      {formatGuestSummary(booking?.guests)}
                                    </Typography>
                                  </Box>
                                </Stack>
                              </TableCell>
                              ...
                              {/* Booking Dates */}
                              <TableCell sx={{ borderBottom: '1px solid', borderColor: 'divider' }}> {booking?.createdAt && format(booking?.createdAt, 'dd MMM, yyyy')}</TableCell>
                              {/* Start & End Dates */}
                              <TableCell sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                                {' '}
                                {booking?.startDate && format(booking?.startDate, 'dd MMM, yyyy')} - {booking?.endDate && format(booking?.endDate, 'dd MMM, yyyy')}
                              </TableCell>
                              {/* Booking Type */}
                              <TableCell align="center" sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                                <Chip
                                  label={booking?.bookingType}
                                  color={booking?.bookingType === 'INQUIRY' ? 'info' : booking?.bookingType === 'REQUEST' ? 'warning' : booking?.bookingType === 'BOOKING' ? 'success' : 'default'}
                                  sx={{
                                    height: 25,
                                    pointerEvents: 'none',
                                    '& .MuiChip-label': {
                              ...
                              <TableCell align="center" sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                                <Chip
                                  ...
                                  color={booking.bookingStatus == 'CONFIRMED' ? 'info' : booking.bookingStatus == 'ACCEPTED' ? 'success' : 'error'}
                                  sx={{
                                    height: 25,
                                    pointerEvents: 'none',
                                    '& .MuiChip-label': {
                              ...
                              <TableCell sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                                {booking?.confirmedAt ? (
                                  format(booking?.confirmedAt, 'dd MMM, yyyy')
                                ) : (
                              ...
                              <TableCell align="center" sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                                <IconButton
                                  component={Link}
                                  href={`/guest/bookings/details/${booking?.id}`}
                                  sx={{
                      ...
                      >
                        {bookingsData.data.map((booking, i) => (
                          <Paper component={Link} href={`/guest/bookings/details/${booking?.id}`} key={i} sx={{ p: { xs: 1, sm: 2 }, borderRadius: 1, boxShadow: '0 4px 20px rgba(0, 0, 0, 0.1)' }}>
                            <Stack direction="row" spacing={{ xs: 1.5, sm: 2 }} alignItems="center">
                              ...
                              >
                                <Image src={booking?.property?.propertyImages[0]?.image} alt={booking?.property?.propertyDescription?.name || 'Property Image'} fill style={{ objectFit: 'cover' }} />
                              </Box>
                              <Box width={'100%'}>
                                ...
                                <Stack direction={'row'} gap={0.5}>
                                  ...
                                  <Typography variant="body2" fontSize={{ xs: 12 }}>
                                    {booking?.createdAt && format(booking?.createdAt, 'dd MMM, yyyy')}
                                  </Typography>
                                </Stack>
                ...
                {bookingsData && bookingsData?.data?.length > 0 && (
                  <Box display="flex" justifyContent="end" p={2}>
                    <Pagination onChange={onDataChange} page={page} count={bookingsData?.pagination?.totalPages} variant="outlined" shape="rounded" />
                  </Box>
                )}
              </TableContainer>
            </TabPanel>
