...
const WeeklyPriceSettings = forwardRef(({ currency, basePrice = 0, value, onChange }, ref) => {
  const { trans } = useTranslation();
  const { activeCurrency, guestFee, hostFee } = useReluxRentAppContext();
  const [weeklyDiscountedPrice, setWeeklyDiscountedPrice] = useState(Math.round(basePrice * 7).toFixed(2));
  ...
  useEffect(() => {
    ...
    if (value !== undefined) {
      ...
      const weeklyBasePrice = Math.round(basePrice * 7).toFixed(2);
      const percent = (weeklyBasePrice * value) / 100;
      setWeeklyDiscountedPrice(Math.round(weeklyBasePrice - percent).toFixed(2));
    }
  }, [value, basePrice]);
  ...
  const handleChange = (_, newValue) => {
    ...
    onChange(newValue);
    const weeklyBasePrice = Math.round(basePrice * 7).toFixed(2);
    const percent = (weeklyBasePrice * newValue) / 100;
    setWeeklyDiscountedPrice(Math.round(weeklyBasePrice - percent).toFixed(2));
  };
  ...
  const { totalGuestFee, totalHostFee, guestTotal, hostTotal } = calculateFees(weeklyDiscountedPrice);
      ...
      >
        <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <Typography sx={(theme) => ({ fontSize: theme.typography.h3.fontSize, fontWeight: 'bold' })}>
            {currency?.symbol || activeCurrency?.symbol} {weeklyDiscountedPrice}
          </Typography>
...
export default WeeklyPriceSettings;
