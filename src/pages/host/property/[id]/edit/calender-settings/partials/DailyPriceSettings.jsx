...
import { Accordion, AccordionDetails, AccordionSummary, Box, Button, Divider, IconButton, Switch, TextField, Typography } from '@mui/material';
...
const DailyPriceSettings = ({ propertyDataState, selectedDate, setSelectedDate }) => {
  console.log('propertyDataState', propertyDataState);
  ...
  console.log(`available (${format(selectedDate, 'dd MMM')}) -`, available);
  ...
  useEffect(() => {
    ...
    const status = propertyDataState?.dateWisePrices[formatted]?.status ?? true;
  ...
  useEffect(() => {
    const formatted = format(selectedDate, 'yyyy-MM-dd');
    const price = propertyDataState?.dateWisePrices[formatted]?.price || propertyDataState?.propertyPrice?.price;
  ...
  const onSubmit = async (formData) => {
    try {
      setLoading(true);
      const { data, status } = await api.put(`/api/host/property/${id}/update/daily-price`, formData);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {available ? (
            <Typography fontSize={14}>{trans('Available')}</Typography>
          ) : (
            <Box>
              <Typography fontSize={14}>{trans('Blocked by you')}</Typography>
            </Box>
          )}
        ...
        <Box sx={{ width: '100%', mx: 'auto' }}>
          <Controller
            ...
            render={({ field }) => {
              ...
              const currencySymbol = propertyDataState?.propertyPrice?.currency?.symbol || '';
          ...
          >
            ...
            >
              ...
              <Typography fontWeight="bold">
                {propertyDataState?.propertyPrice?.currency?.symbol || activeCurrency?.symbol}
            ...
            <AccordionDetails sx={{ px: 0, pt: 1 }}>
              ...
              <Box display="flex" justifyContent="space-between" mb={1}>
                ...
                <Typography fontWeight="bold">
                  {propertyDataState?.propertyPrice?.currency?.symbol || activeCurrency?.symbol} {guestFeeTotal}
                </Typography>
              </Box>
...
export default DailyPriceSettings;
