...
import { Box, Button, capitalize, IconButton, Stack, Typography, useMediaQuery } from '@mui/material';
...
function groupedAmenities(amenities) {
  return amenities.reduce((acc, amenity) => {
    const typeName = amenity.amenitiesType.translations.name;
    if (!acc[typeName]) {
      acc[typeName] = [];
    }
    acc[typeName].push({
...
const Amenities = ({ amenities, propertyData }) => {
  const { user } = useAuth();
  ...
  const groupAmenities = groupedAmenities(amenities);
  ...
  const {
    ...
    defaultValues: {
      amenities: propertyData?.amenities?.map((aminity) => aminity.id) || [],
    },
  });

  const watchSelected = watch('amenities') || [];

  const conditionalMarginLeft = propertyData.allStepsCompleted ? { md: '400px' } : {};

  const onSelectAmenity = (id) => {
    const newSelection = watchSelected.includes(id) ? watchSelected.filter((i) => i !== id) : [...watchSelected, id];
    setValue('amenities', newSelection, { shouldValidate: true });
  };

  const onSubmit = async (formData) => {
    try {
      setLoading(true);
      const { data, status } = await api.put(`/api/host/property/${id}/update/amenities`, formData);
      if (status == 201 && !propertyData.allStepsCompleted) {
        router.push(`/host/property/${id}/edit/photos?tab=space`);
      }
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <Stack width={'100%'} gap={4}>
                  {Object.entries(groupAmenities).map(([resource, perms]) => (
                    <Box key={resource}>
                      ...
                      <Box>
                        ...
                        >
                          {perms.map((item) => {
                            ...
                            const isSelected = watchSelected.includes(item.id);
                            return (
                              <Box
                                key={item.id}
                                sx={{
          ...
          >
            {propertyData.allStepsCompleted ? (
              ...
              >
                <Button
                  component={Link}
                  href={`/host/property/${id}/edit/floor-plan?tab=space`}
                  size="small"
...
export async function getServerSideProps(context) {
  try {
    ...
    const [amenities, propertyData] = await Promise.all([
      api.get(`/api/host/amenities?lang=${lang.code}`, { headers: { Cookie: cookies } }).then((res) => res.data || []),
