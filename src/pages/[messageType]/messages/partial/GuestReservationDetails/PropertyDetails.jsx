...
export default function PropertyDetails({ details }) {
  ...
  const handleConfirmDeclineByGuest = async () => {
    try {
      setLoading(true);
      const { data } = await api.put(`/api/guest/decline-booking-request/${lastBooking?.id}`, { conversationBookingId: details?.conversationBookingId });
              ...
              {details?.reviewFromHost && (
                <>
                  ...
                  <Typography variant="body2" fontWeight={600}>
                    {lastBooking?.host?.name}'s review:
                  </Typography>
                  {/* Last review message */}
                  <Stack
                    direction="row"
                  ...
                  >
                    ...
                    >
                      {details?.reviewFromHost?.message}
                    </Typography>
                  </Stack>

                  <Typography onClick={() => actions.openPopup('showHostReviews', details)} variant="body2" fontWeight={600} sx={{ textDecoration: 'underline', cursor: 'pointer' }}>
                    {trans('Show reviews')}
                  </Typography>
                </>
              )}
