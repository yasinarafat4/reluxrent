...
export default function ShowHostReviewsPopup({ closeModal, showModal, popupData }) {
  ...
  return (
    ...
    >
      <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'} pr={1}>
        <DialogTitle sx={{ fontSize: { xs: '14px', md: '17px', fontWeight: 600 } }}>What hosts say about {popupData?.lastBooking?.guest?.name}</DialogTitle>
        <IconButton
          aria-label="close"
          onClick={closeModal}
          sx={(theme) => ({
      ...
      <DialogContent>
        <Stack spacing={4} px={{ xs: 1, sm: 2 }}>
          {reviews.length > 0 ? (
            ...
            <NoDataFound title="No Reviews Yet!" subtitle="This guest hasn't received any reviews from hosts so far." />
          )}
        </Stack>
      </DialogContent>
    </Dialog>
