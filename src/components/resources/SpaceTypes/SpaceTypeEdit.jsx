...
import SpaceTypeEditToolbar from './SpaceTypeEditToolbar';

const SpaceTypeEditTitle = () => {
  const appTitle = useDefaultTitle();
  const { defaultTitle } = useEditContext();
  return <title>{`${defaultTitle}-${appTitle}`}</title>;
};

const SpaceTypeEditActions = () => (
  <TopToolbar>
    <CloneButton />
...
const SpaceTypeEdit = () => {
  return (
    <Edit title={<SpaceTypeEditTitle />} actions={<SpaceTypeEditActions />} mutationMode="pessimistic">
      <SimpleForm toolbar={<SpaceTypeEditToolbar />}>
