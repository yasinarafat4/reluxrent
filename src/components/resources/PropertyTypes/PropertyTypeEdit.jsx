...
import PropertyTypeEditToolbar from './PropertyTypeEditToolbar';

const PropertyTypeEditTitle = () => {
  const appTitle = useDefaultTitle();
  const { defaultTitle } = useEditContext();
  return <title>{`${defaultTitle}-${appTitle}`}</title>;
};

const PropertyTypeEditActions = () => (
  <TopToolbar>
    <CloneButton />
...
const PropertyTypeEdit = () => {
  return (
    <Edit title={<PropertyTypeEditTitle />} actions={<PropertyTypeEditActions />} mutationMode="pessimistic">
      <SimpleForm toolbar={<PropertyTypeEditToolbar />}>
