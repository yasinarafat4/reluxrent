      ...
      {languages.map((lang) =>
        currentLocale === lang.code ? (
          <Box key={lang.code} sx={{ display: 'flex', flexDirection: 'column', gap: 1, width: '100%' }}>
            <TextInput source={`${source}.${lang.code}.title`} label={`Page Title (${lang.name})`} fullWidth validate={[required(), minLength(3)]} />
            <AutoSlugField source="slug" fromLang="en" fullWidth />
            <TinyMCEInput source={`${source}.${lang.code}.content`} label={`Page Content (${lang.name})`} fullWidth />
