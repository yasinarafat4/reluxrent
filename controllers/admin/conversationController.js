...
exports.getAllConversation = async (req, res) => {
  try {
    ...
    // Map conversations for admin dashboard
    const conversationsData = conversations.map((conv) => {
      const otherParticipants = conv.participants.map((p) => p.participant);
      ...
      const property = {
        id: conv.property.id,
        propertyDescription: { name: translatedDescriptions?.name },
...
exports.getConversationById = async (req, res) => {
  try {
    ...
    const participants = conversation.participants.map((p) => p.participant);
    ...
    const { guestStatus, reviewDeadline, canReview, reviewFromGuest, reviewFromHost } = await getGuestStatus(lastBooking?.booking, lastBooking?.booking.guest, isAvailable);
    ...
    res.json(
      convertDecimals({
        ...
        conversationbookings: conversation.conversationbookings,
